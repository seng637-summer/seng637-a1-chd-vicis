...
The group performed MFT in the same pair groups but with the first group doing test cases 1-20 while the second group performed cases 21-40.  As new bugs were found they were first compared to the existing list and added only if they were unique.  After going through all the test cases the group met as a whole to discuss issues and provide feedback (noted in the section above).

Regression testing was carried out in a similar mannger to MFT with the groups dividing the issues evenly and retesting them on the new ATM version 1.1.  If the issue was resolved it was marked as such in Jira and if it was not resolved a comment was left stating it still exists in version 1.1.  Comments included any difference between the systems if the issue was not resolved, for example A1-6 was not resolved and instead gave a different outcome.
